...
export function buildRecognitionTableColumns(t, options = {}) {
  ...
  const cols = [
    ...
    col({ title: titleFor('NO', 'taskEdit.workerNumber'), dataIndex: 'NO', key: 'NO', maxWidth: 96, minWidth: 88, align: 'center', customCell: bindCellStyle(cellStyle, 'NO') }),
    col({ title: t('taskEdit.employeeNo'), dataIndex: 'EMPLOYEE_NO', key: 'EMPLOYEE_NO', customCell: bindCellStyle(cellStyle, 'EMPLOYEE_NO') }),
    requiredCol('Pays', { title: titleFor('Pays', 'taskEdit.countryField'), dataIndex: 'Pays', key: 'Pays', maxWidth: 84, minWidth: 60, customCell: bindCellStyle(cellStyle, 'Pays') }),
