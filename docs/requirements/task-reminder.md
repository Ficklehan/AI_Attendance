...
| 提醒人 | **管理员在规则中自由指定**（多选用户），不默认绑定任务创建者 |
...
- 所有规则配置的「提醒人」均写入 `user_notifications`
...
4. 若 `now - status_entered_at >= 周期`，且本周期桶内该 `(rule_id, task_id, user_id)` 未投递，则触发提醒  
5. 任务状态离开规则目标状态（如 `processed → confirmed`）后，**不再提醒**  
6. 同一规则对同一任务：每满一个周期可再提醒一次（更新文案中的 `pendingCount` 等聚合信息）
...
1. 管理员可创建一条规则：状态=`processed`，周期=1天，指定≥1提醒人，自定义文案。  
2. 存在超过 1 天未确认的 `processed` 任务时，调度执行后提醒人收到站内消息；有飞书 ID 的另收飞书消息。  
